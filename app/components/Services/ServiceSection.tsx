...
export default function ServiceSection() {
        ...
        const normalized = (data || []).map((s) => ({
          ...
          imageUrl: s.imageUrl
            ? s.imageUrl.startsWith("http")
              ? s.imageUrl
              : `${SERVER_BASE_URL}${s.imageUrl.startsWith("/") ? "" : "/"}${s.imageUrl}`
            : "",
  ...
  const translatePx = containerWidth ? Math.max(maxTranslate, Math.min(0, rawTranslate)) : 0;
    ...
    <section className="py-16 bg-gray-50 relative">
      <div className="max-w-7xl mx-auto px-6 relative">
          ...
          <div ref={viewportRef} className="overflow-hidden mx-6 sm:mx-12 w-full" style={{ minHeight: 340 }}>
            ...
            >
              {services.map((service, idx) => (
                <div key={service.id} className="flex-shrink-0 px-3" style={{ width: itemWidth || "100%" }}>
                    ...
                    >
                      <div>
                        ...
                        <p className="text-gray-600 text-sm leading-relaxed text-center break-words whitespace-pre-line">
                          {service.shortDesc || "No short description available."}
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <div className="text-center mt-12">
          <Link
            href="/services"
            className="inline-block bg-blue-800 text-white px-8 py-4 rounded-lg font-semibold hover:bg-blue-900 transition-all duration-300 transform hover:scale-105 hover:shadow-lg min-w-[200px]"
          >
            View All Services
          </Link>
        </div>
