...
interface Achievement {
  id: number;
  title: string;
  imageUrl?: string | null; // sekarang pakai URL langsung dari API
  createdAt: string;
}
...
export default function AchievementManagementPage() {
  ...
  const fetchAchievements = async () => {
    try {
      ...
      // sort createdAt (paling lama dulu)
  ...
  const currentAchievements = filtered.slice(startIndex, startIndex + itemsPerPage);
        ...
        >
          <div className="max-w-screen-xl mx-auto pb-8">
            ...
            <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
              <h2 className="text-xl md:text-3xl font-bold text-gray-900">Achievement Management</h2>
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                {currentAchievements.length > 0 ? (
                  currentAchievements.map((achievement) => (
                    <div key={achievement.id} className="bg-white rounded-xl shadow p-6 text-center">

                      <div className="bg-white p-3 rounded-lg flex justify-center items-center h-36 mb-4 shadow-inner">
                        <img
                          src={achievement.imageUrl || "/placeholder.png"} // langsung pakai URL
              ...
              <div className="px-6 py-4 flex flex-col sm:flex-row items-center justify-between border-t border-gray-200">
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.min(startIndex + currentAchievements.length, filtered.length)}</span>{" "}
